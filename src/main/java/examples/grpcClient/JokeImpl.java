...
class JokeImpl extends JokeGrpc.JokeImplBase {
    ...
    public void getJoke(JokeReq req, StreamObserver<JokeRes> responseObserver) {
        ...
        for (int i=0; i < req.getNumber(); i++){
            if(!jokes.empty()) {
                response.addJoke(jokes.pop()); // yes, I take the joke out when it was used already,
                //should probably be done differently since this way a joke cannot be told twice even to different clients
            }
            else {
                response.addJoke("I am out of jokes..."); // this is more of a hack,
                //better would be to either check the number
                //at the beginnig and say right away if you do not have enough.
                //Or send an error code or similar as well.
                break;
            }
        }
